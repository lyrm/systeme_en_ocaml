  ...
  let find on_error on_path follow depth roots =
    let rec find_rec depth visiting filename =
      try
        ...
        if infos.st_kind = Unix.S_DIR && depth > 0 && continue &&
      ...
      with Unix.Unix_error (e, b, c) -> hide_exn on_error (e, b, c) in
...
let getcwd () =
  let get_id name = let s = Unix.stat name in s.st_dev, s.st_ino in
  ...
  let rec build current_path path_id = match path_id with
    | [] -> current_path
    | parent :: xs ->
       let found = ref "" in
       iter_dir (fun elt ->
           try
             if get_id (Filename.concat current_path elt) = parent
             then found := Filename.concat current_path elt
...
module IO : sig
  ...
  let input_char chan =
    if chan.in_pos < chan.in_end then begin
        ...
        match Unix.read chan.in_fd chan.in_buffer 0 buffer_size
