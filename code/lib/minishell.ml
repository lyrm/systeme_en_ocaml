...
let setup_redirection =
  ...
  | Ast.In target ->
    ...
    Unix.close fd
  | Ast.Out target ->
    let fd = Unix.openfile target [Unix.O_CREAT; Unix.O_TRUNC; Unix.O_WRONLY] 0o660 in
...
let exec_cmd = function 
  ...
  | Ast.External ((program :: _) as cmd) -> Unix.execvp program (Array.of_list cmd)
  | _ -> failwith ""
